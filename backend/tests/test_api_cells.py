...
class TestGetCellEarthquakes:
    ...
    async def test_returns_earthquakes_in_cell_sorted_desc(self, client, db_session):
        ...
        db_session.add_all(
            [
                Earthquake(
                    id="old", latitude=35.6, longitude=139.7, depth_km=10.0, magnitude=3.0,
                    occurred_at=older, depth_layer=cell.depth_layer, lat_band=cell.lat_band,
                    lon_index=cell.lon_index,
                ),
                Earthquake(
                    id="new", latitude=35.6, longitude=139.7, depth_km=10.0, magnitude=4.0,
                    occurred_at=newer, depth_layer=cell.depth_layer, lat_band=cell.lat_band,
                    lon_index=cell.lon_index,
                ),
            ]
        )
        ...
        resp = await client.get(
            f"/api/cells/{cell.depth_layer}/{cell.lat_band}/{cell.lon_index}"
        )
